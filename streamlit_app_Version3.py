...
if not c.fetchone():
    c.execute("INSERT INTO users VALUES (NULL,?,?,?,?)",
              ("boss",hash_pw("boss123"),"boss",datetime.datetime.utcnow().isoformat()))
    db.commit()
...
# ================= MAIN =================
    ...
    # -------- PRODUK & STOK --------
        ...
        if st.button("Ambil"):
            c.execute("UPDATE products SET warehouse_stock=warehouse_stock-?, stock=stock+? WHERE id=?",
                      (qty,qty,pid))
            db.commit(); st.rerun()
        st.dataframe(df[["sku","name","stock"]])

    # -------- PENJUALAN (GRID PRODUK) --------
    elif menu=="Penjualan":
        st.header("Penjualan")
        df=products()
    ...
    # -------- HISTORI --------
        ...
        if role=="boss":
            df=pd.read_sql("""SELECT s.id,p.name,s.qty,s.total,s.profit,s.sold_at
                              FROM sales s JOIN products p ON s.product_id=p.id""",db)
        else:
            df=pd.read_sql("""SELECT s.id,p.name,s.qty,s.total,s.sold_at
                              FROM sales s JOIN products p ON s.product_id=p.id""",db)
        st.dataframe(df)
    ...
    # -------- USER --------
        ...
        with st.form("user"):
            ...
            if st.form_submit_button("Tambah"):
                try:
                    c.execute("INSERT INTO users VALUES (NULL,?,?,?,?)",
                              (u,hash_pw(p),r,datetime.datetime.utcnow().isoformat()))
                    db.commit(); st.rerun()
                except:
